...
class Transformer(nn.Module):
    ...
    _GDRIVE_CHECKPOINT_ID = "1WbFlO1qem_mNl_4lrVKecmPTDp9qyu2e"
    ...
    def __init__(
        ...
        # ── Step 3: Optionally download and load a pre-trained checkpoint ──
    ...
    def _build_vocabularies(
        ...
        try:
            ...
            # Add tokens that appear at least twice (min-freq filter).
            for word, freq in german_freqs.items():
                if freq >= 2 and word not in self.src_token_to_idx:
                    self.src_token_to_idx[word] = len(self.src_token_to_idx)

            for word, freq in english_freqs.items():
            ...
            # Non-fatal: the model architecture will still be built; weights
            # loaded from a checkpoint will restore proper embeddings.
    ...
    def _load_pretrained_checkpoint(self, output_path: str) -> None:
        """
        Download the best pre-trained checkpoint from Google Drive and
        load its weights into this model.

        Handles key name differences between the saved checkpoint and the
        current model by applying a set of known renames.

        Args:
            output_path : Local file path to save the downloaded checkpoint.
        """
        ...
        try:
            ...
            # Apply known key renames from older checkpoint conventions.
            remapped_state_dict = {}
            ...
            key_rename_rules = {
                ...
                "src_embed.1.":  "src_positional_enc.",
                "tgt_embed.1.":  "tgt_positional_enc.",
                "generator.":    "vocab_projection.",
                "w_q.":          "proj_query.",
                "w_k.":          "proj_key.",
                "w_v.":          "proj_value.",
                "w_o.":          "proj_out.",
                "src_attn.":     "cross_attention.",
            ...
            for ckpt_key, param_tensor in raw_state_dict.items():
                normalised_key = ckpt_key
                for old_pattern, new_pattern in key_rename_rules.items():
                    normalised_key = normalised_key.replace(old_pattern, new_pattern)

                if normalised_key not in current_keys:
                    continue  # Skip keys not present in the current architecture.

                # Handle shape mismatches by copying the overlapping slice.
                current_shape = self.state_dict()[normalised_key]
                if param_tensor.shape != current_shape.shape:
                    patched = current_shape.clone()
                    if param_tensor.dim() == 2 and current_shape.dim() == 2:
                        r = min(param_tensor.size(0), current_shape.size(0))
                        c = min(param_tensor.size(1), current_shape.size(1))
                        patched[:r, :c] = param_tensor[:r, :c]
                    elif param_tensor.dim() == 1 and current_shape.dim() == 1:
                        n = min(param_tensor.size(0), current_shape.size(0))
                        patched[:n] = param_tensor[:n]
                    remapped_state_dict[normalised_key] = patched
                else:
                    remapped_state_dict[normalised_key] = param_tensor

            self.load_state_dict(remapped_state_dict, strict=False)
            print("[Transformer] Pre-trained checkpoint loaded successfully.")

        except Exception as load_err:
            print(f"[Transformer] Checkpoint load error: {load_err}")
